...
black_runner.py
  ...
  ├─ attacks/date_patch/poc_clock_rotate.py     ├─→ black_findings.json ─┐
  ├─ attacks/crack_license/poc_license_forgery.─┘                        ▼
  └─ (circumvent: 수동 절차)                                            report_sec.py
                                                                        → security_report.html (blue 핸드오프)
```

## 6. 윤리 가드레일 (HARD)
- **본인 소유 앱 · 의도적 취약 랩** 외 실행 금지 (`--i-own-this` 필수).
- **제공 거부**: 배포용 크랙/keygen/범용 패처·DoS 도구·실제 상용 소프트 우회 이진.
- 타인 시스템·서비스 공격·저작물 복제 금지. 모든 PoC = **원리 시연용 generic**.
- 이 팀의 궁극적 목적은 **안티태퍼 평가(소프트웨어 보호)** — “뚫리는 법”을 알고
  “막는 법(Blue)”으로 연결하는 방어 중심 연구.
...
└── attacks/
    ...
    ├── crack_license/      # 🔹 라이선스 파일 위조·검증 우회
