...
plt=sect(".plt.sec"); gotplt=sect(".got.plt"); dynsym=sect(".dynsym"); dynstr=sect(".dynstr")
...
if aesidx is not None:
    ...
    aesgotf = gotplt["off"] + (aesgotoff - gotplt["addr"])
    ...
    def find_stub_for_got(gotf):
        ...
        for k in range(3, n):
            ...
            gotaddr = s+6+rel
            gotfoff = gotaddr - gotplt["addr"] + gotplt["off"]
            if gotfoff==gotf:
                return k, s
        return None
