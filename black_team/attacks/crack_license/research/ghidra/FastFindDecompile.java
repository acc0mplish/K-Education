...
public class FastFindDecompile extends GhidraScript {
    ...
    // True decompiler success: decompileCompleted() just means "no error queued",
    // so it is a false positive. A real result requires a DecompiledFunction with
    // non-empty C text. isValid() == decompileCompleted() && getC() not empty.
    private static boolean hasC(DecompileResults r) {
        if (r == null || !r.decompileCompleted()) return false;
        DecompiledFunction df;
        try { df = r.getDecompiledFunction(); } catch (Exception e) { return false; }
    ...
    public void run() throws Exception {
        ...
        //    background DecompilerSwitchAnalyzer has released its lock. Test a
        //    Rust-code function (not the low PE-import-trampoline region) so the
        //    warmup is representative of the real targets. Retries handle the
        //    transient switch-analyzer lock; gives up after ~60s on a dead binary.
        // NOTE: run with `-noanalysis` (see reproduce_fastfind.sh). Without it, the
        // switch-decompiler analysis runs during import and disposes/restarts the
        // native decompiler process, so this postScript's process never launches
        // (getC() is null -> decompiled=0). -noanalysis keeps the process ready.
        ...
        for (Function cand : all) {
            String nn = cand.getName();
            // stripped binary: Rust 함수는 FUN_ 이 아닌 이름 (비교적 낮은 주소 밴드).
            if (nn != null && !nn.startsWith("FUN_")) { warmup = cand; break; }
        }
        boolean decOK = false;
        if (warmup != null) {
            for (int w = 0; w < 24; w++) {
                ...
                if (wr != null && hasC(wr)) { decOK = true; break; }
