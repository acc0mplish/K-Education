# 상용 소프트웨어 License Crypto 분석 — Seafile Pro & Mattermost
...
- **Seafile Pro**: license = **X509 certificate + RSA** (AES-128-CBC 아님).
  AES-128-CBC(`EVP_aes_128_cbc`)는 **파일/저장소 암호화**에만 쓰임.
- **Mattermost**: license = **RSA PKCS1v15 + SHA-512 서명 검증**.
- 둘 다 **하드코드 RSA 공개키**로 license 서명 검증.
- **private key(서명용)**는 발급자(Ministry/Seafile/Mattermost) 빌드에만 있음.
  → OSS/binary에서 **patch(검증 우회)**가 유일한 활성화 경로.

---

## 1. Seafile Pro (`seaf-server`, 64-bit PIE, stripped ELF)
...
### 1.3 Seafile 활성화 (비활성화) 방법

1. `0x612a0` / `0x60330`에서 `EVP_verify` 결과를 **강제 1(success)**로 패치 — **키 없이도 가능, 가장 간단**.
2. 또는 Seafile production **private key**로 license 인증서 forgery (public key 위 대로).
...
| private key | Seafile 빌드에만 | mattermost 프로프 빌드에만 |
| 활성화(패치) | `EVP_verify` → 1로 덮기 | `ValidateLicense` 서명 체크 우회 |
| 활성화(복제) | Seafile private key로 인증서 forgery | mattermost private key로 license 서명 |

---

## 4. 산출물 위치 (모두 /tmp)

- Seafile findings: `/tmp/ghwork/FINDINGS.txt`, RSA key: `/tmp/ghwork/rsa.key`
- Mattermost public keys: 위 소스 경로 (K-Education SOURCE repo).
