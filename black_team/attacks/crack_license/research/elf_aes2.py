...
for si in range(nsym):
    if str_at(struct.unpack_from("<I",data,sec(dynsym)["off"]+si*entsize)[0])=="EVP_aes_128_cbc":
        target=si; break
# search every .rela section for symbol index target
for i in range(e_shnum):
    ...
    for ri in range(nre):
        ...
        r_info=U8(base+8); symidx=r_info & 0xffffffff
        if symidx==target:
            r_offset=U8(base); addend=struct.unpack_from("<q",data,base+16)[0]
            # section this offset belongs to
            secidx=None
            for j in range(e_shnum):
                s=sec(j)
                if s["off"]<=r_offset<s["off"]+s["size"]:
                    secidx=j
            print(f"{nm}: r_offset={hex(r_offset)} addend={addend} -> section {secnames[secidx] if secidx is not None else '?'}")
