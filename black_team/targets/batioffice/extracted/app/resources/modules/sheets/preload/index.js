...
const SAVE_EDITS_CHUNK_MAX = 1e5;
...
const IPC_CHANNELS = {
  ...
  aiProbeProvider: "ai:probe-provider",
  aiOllamaHostInfo: "ai:ollama-host-info",
  aiOllamaRuntimeStatus: "ai:ollama-runtime-status",
  aiOllamaRuntimeStart: "ai:ollama-runtime-start",
  aiOllamaRuntimeInstall: "ai:ollama-runtime-install",
  aiOllamaPull: "ai:ollama-pull",
  aiOllamaPullCancel: "ai:ollama-pull-cancel",
  aiOllamaPullProgress: "ai:ollama-pull-progress",
  aiChat: "ai:chat",
  aiStream: "ai:stream",
  aiStreamCancel: "ai:stream-cancel",
  aiStreamChunk: "ai:stream-chunk",
  aiBatiLogin: "ai:bati-login",
...
const desktopApi = {
  ...
  async sendSaveEditsChunk(request) {
    ...
    if (!Array.isArray(request.edits) || request.edits.length === 0 || request.edits.length > SAVE_EDITS_CHUNK_MAX) {
  ...
  async exportPdf(request) {
    if (!isRecord(request) || typeof request.fileName !== "string" || request.fileName.length === 0 || request.fileName.length > 255 || typeof request.html !== "string" || request.html.length === 0 || request.html.length > 2e7 || typeof request.landscape !== "boolean" || !isPdfPageSize(request.pageSize) || !isRecord(request.margins) || !["top", "bottom", "left", "right"].every((edge) => {
      const value = request.margins[edge];
      return typeof value === "number" && value >= 0 && value <= 3;
    }) || typeof request.scale !== "number" || request.scale < 0.1 || request.scale > 2 || request.headerTemplate !== void 0 && !isBoundedString(request.headerTemplate, 5e4) || request.footerTemplate !== void 0 && !isBoundedString(request.footerTemplate, 5e4)) {
  ...
  onMenuAction(callback) {
    const listener = (_event, action) => {
      if (action === "open" || action === "save" || action === "save-as" || action === "export-pdf" || action === "export-csv" || action === "undo" || action === "redo")
        callback(action);
    };
  ...
  async probeAiProvider(settings) {
    return electron.ipcRenderer.invoke(IPC_CHANNELS.aiProbeProvider, settings);
  },
  async getOllamaHostInfo() {
    return electron.ipcRenderer.invoke(IPC_CHANNELS.aiOllamaHostInfo);
  },
  async getOllamaRuntimeStatus() {
    return electron.ipcRenderer.invoke(IPC_CHANNELS.aiOllamaRuntimeStatus);
  },
  async startOllamaRuntime() {
    return electron.ipcRenderer.invoke(IPC_CHANNELS.aiOllamaRuntimeStart);
  },
  async installOllamaRuntime() {
    await electron.ipcRenderer.invoke(IPC_CHANNELS.aiOllamaRuntimeInstall);
  },
  async pullOllamaModel(request) {
    await electron.ipcRenderer.invoke(IPC_CHANNELS.aiOllamaPull, request);
  },
  async cancelOllamaPull(requestId) {
    await electron.ipcRenderer.invoke(IPC_CHANNELS.aiOllamaPullCancel, requestId);
  },
  onOllamaPullProgress(callback) {
    const listener = (_event, progress) => {
      callback(progress);
    };
    electron.ipcRenderer.on(IPC_CHANNELS.aiOllamaPullProgress, listener);
    return () => electron.ipcRenderer.removeListener(IPC_CHANNELS.aiOllamaPullProgress, listener);
  },
  ...
  async aiBatiLogin() {
    await electron.ipcRenderer.invoke(IPC_CHANNELS.aiBatiLogin);
  },
  async aiBatiAccountSummary() {
    return await electron.ipcRenderer.invoke("batioffice:account:summary");
  },
  onAiBatiAccountChanged(callback) {
  ...
  async consumeAiPrompt() {
    const result = await electron.ipcRenderer.invoke("sheets:consume-ai-prompt");
...
function parseWorkbookFile(input) {
  ...
  if (!isUuid(sessionId) || typeof name !== "string" || path !== void 0 && (typeof path !== "string" || path.length === 0) || typeof sha256 !== "string" || !/^[a-f0-9]{64}$/.test(sha256) || !isNonnegativeInteger(entryCount) || !Array.isArray(sheets) || !Array.isArray(styles) || !Array.isArray(dxfStyles) || !Array.isArray(visuals) || !Array.isArray(definedNames) || typeof readOnly !== "boolean" || needsSaveAs !== void 0 && typeof needsSaveAs !== "boolean" || csvPath !== void 0 && (typeof csvPath !== "string" || csvPath.length === 0) || restoredFromRecovery !== void 0 && typeof restoredFromRecovery !== "boolean") {
...
function parseRangeRequest(input) {
  ...
  if (!isNonnegativeInteger(startRow) || !isNonnegativeInteger(endRow) || !isNonnegativeInteger(startColumn) || !isNonnegativeInteger(endColumn) || startRow > endRow || startColumn > endColumn || (endRow - startRow + 1) * (endColumn - startColumn + 1) > 2e4) {
    throw new Error("Invalid workbook range request.");
  }
  return input;
}
function parseCellRecord(cell) {
  if (!isRecord(cell) || !isNonnegativeInteger(cell.row) || !isNonnegativeInteger(cell.column) || !isCellScalar(cell.value) || cell.formula !== void 0 && typeof cell.formula !== "string" || cell.styleIndex !== void 0 && !isNonnegativeInteger(cell.styleIndex)) {
...
function parseRecalcResult(input) {
  ...
  const cells = input.cells.map((cell) => {
    if (!isRecord(cell) || typeof cell.sheetId !== "string" || cell.sheetId.length === 0 || !isNonnegativeInteger(cell.row) || !isNonnegativeInteger(cell.column) || typeof cell.formatted !== "string" || cell.number !== void 0 && (typeof cell.number !== "number" || !Number.isFinite(cell.number)) || typeof cell.isFormula !== "boolean") {
      throw new Error("Invalid workbook recalc response.");
    }
    return {
...
function parseRangeResult(input) {
  if (!isRecord(input) || !Array.isArray(input.cells) || input.cells.length > 2e4 || !Array.isArray(input.rows) || input.rows.length > 2e4 || !Array.isArray(input.merges) || input.merges.length > 2e4 || !Array.isArray(input.hyperlinks) || input.hyperlinks.length > 2e4 || !Array.isArray(input.conditionalRules) || input.conditionalRules.length > 2e4 || input.indexedThroughRow !== null && !isNonnegativeInteger(input.indexedThroughRow) || typeof input.indexingComplete !== "boolean") {
  ...
  if (!Array.isArray(input.dataValidations) || input.dataValidations.length > 2e4 || input.autoFilter !== null && !isRecord(input.autoFilter)) {
    throw new Error("Invalid workbook range response.");
  }
...
function parseConditionalRule(input) {
  ...
  if (!isOptionalString(input.iconSetName) || typeof input.iconReverse !== "boolean" || typeof input.showValue !== "boolean" || !isOptionalString(input.negativeColor) || input.negativeSameAsPositive !== void 0 && typeof input.negativeSameAsPositive !== "boolean" || input.gradient !== void 0 && typeof input.gradient !== "boolean") {
    throw new Error("Invalid workbook conditional rule.");
  }
  return {
...
function isFilterColumn(input) {
  ...
  if (input.values !== void 0 && (!Array.isArray(input.values) || input.values.length > 1e4 || input.values.some((value) => typeof value !== "string" || value.length > 255))) {
    return false;
  }
...
function parsePivotTableInfos(input) {
  ...
  return input.map((entry) => {
    if (!isRecord(entry) || typeof entry.path !== "string" || entry.path.length === 0 || entry.cachePath !== null && typeof entry.cachePath !== "string" || typeof entry.outputRef !== "string" || entry.outputRef.length === 0 || entry.headerFill !== void 0 && typeof entry.headerFill !== "string" || entry.headerFontColor !== void 0 && typeof entry.headerFontColor !== "string" || entry.wholeTableFill !== void 0 && typeof entry.wholeTableFill !== "string" || entry.stripeFill !== void 0 && typeof entry.stripeFill !== "string" || entry.styled !== void 0 && typeof entry.styled !== "boolean" || entry.firstDataRow !== void 0 && !isNonnegativeInteger(entry.firstDataRow) || entry.rowGrandTotals !== void 0 && typeof entry.rowGrandTotals !== "boolean") {
      throw new Error("Invalid worksheet pivot metadata.");
    }
    return {
      path: entry.path,
      ...
      ...entry.headerFill === void 0 ? {} : { headerFill: entry.headerFill },
      ...entry.headerFontColor === void 0 ? {} : { headerFontColor: entry.headerFontColor },
      ...entry.wholeTableFill === void 0 ? {} : { wholeTableFill: entry.wholeTableFill },
      ...entry.stripeFill === void 0 ? {} : { stripeFill: entry.stripeFill },
...
function parseVisualObject(input) {
  if (!isRecord(input) || typeof input.id !== "string" || typeof input.sheetId !== "string" || !["chart", "image", "shape"].includes(String(input.kind)) || !isRecord(input.anchor)) {
  ...
  if (!isOptionalString(input.shapeType) || input.opacity !== void 0 && (typeof input.opacity !== "number" || !Number.isFinite(input.opacity) || input.opacity < 0 || input.opacity > 1) || !isOptionalString(input.fillMediaPath) || !isOptionalString(input.fillMediaType) || !isOptionalString(input.fillColor) || !isOptionalString(input.lineColor) || input.lineWidth !== void 0 && (typeof input.lineWidth !== "number" || !Number.isFinite(input.lineWidth)) || !isOptionalString(input.lineDash) || !isOptionalString(input.lineCap) || input.flipH !== void 0 && typeof input.flipH !== "boolean" || input.flipV !== void 0 && typeof input.flipV !== "boolean" || !isOptionalString(input.textColor) || !isOptionalString(input.textAnchor) || !isOptionalString(input.text) || input.rotation !== void 0 && (typeof input.rotation !== "number" || !Number.isFinite(input.rotation)) || input.frameWidth !== void 0 && (typeof input.frameWidth !== "number" || !Number.isFinite(input.frameWidth) || input.frameWidth <= 0) || input.frameHeight !== void 0 && (typeof input.frameHeight !== "number" || !Number.isFinite(input.frameHeight) || input.frameHeight <= 0) || input.drawingPath !== void 0 && (typeof input.drawingPath !== "string" || !/^xl\/drawings\/[A-Za-z0-9._/-]+\.xml$/.test(input.drawingPath)) || input.drawingIndex !== void 0 && (!isNonnegativeInteger(input.drawingIndex) || input.drawingIndex > 1e4)) {
    throw new Error("Invalid workbook visual response.");
  }
  return {
    id: input.id,
...
function parseShapeParagraphs(input) {
  ...
  return input.map((paragraph) => {
    if (!isRecord(paragraph) || !isOptionalString(paragraph.align) || !Array.isArray(paragraph.runs)) {
      throw new Error("Invalid workbook shape paragraph.");
    }
    const runs = paragraph.runs.map((run) => {
      if (!isRecord(run) || typeof run.text !== "string" || !isOptionalString(run.color) || run.bold !== void 0 && typeof run.bold !== "boolean" || run.italic !== void 0 && typeof run.italic !== "boolean" || run.underline !== void 0 && typeof run.underline !== "boolean" || run.size !== void 0 && (typeof run.size !== "number" || !Number.isFinite(run.size))) {
